...
interface FooterProps {
  ...
  footer?: {
    ...
    columns?: Array<{ title: string; links?: Array<{ label: string; url: string }> | null }> | null
  ...
  siteSettings?: {
    socialMedia?: {
      ...
      tiktok?: string | null; linkedin?: string | null
      twitter?: string | null; facebook?: string | null
    } | null
  }
  locale: Locale
  dict: Dictionary
...
export default function Footer({ global, footer, siteSettings, locale, dict }: FooterProps) {
  const t = dict.footer
  ...
  const cmsFooterLinks: FooterLink[] = footer?.columns?.length
    ? footer.columns.flatMap((col) => col.links ?? [])
  ...
  const copyrightText =
    footer?.copyrightText ?? `© ${new Date().getFullYear()} ${global?.siteName ?? 'My App'}. All rights reserved.`
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-12 md:gap-8 items-start">
          ...
          <div className="flex flex-col gap-4">
            {footerLogo ? (
              ...
              <span className="text-2xl font-serif font-bold">{global?.siteName ?? 'My App'}</span>
            )}
            <p className="text-white/60 text-sm leading-relaxed max-w-xs">{t.builtWith}</p>
          </div>

          {/* Navigation */}
          <div className="flex flex-col gap-3">
            <h3 className="font-semibold text-white/80 uppercase tracking-wider text-xs mb-2">{t.navigation}</h3>
            {footerLinks.map((l) => (
              <Link key={l.href} href={l.href} className="text-white/60 hover:text-white transition-colors text-sm">
                {l.label}
              </Link>
            ))}
          </div>
          ...
          {/* Social */}
          <div className="flex flex-col gap-4">
            <h3 className="font-semibold text-white/80 uppercase tracking-wider text-xs">{t.followUs}</h3>
            <div className="flex gap-4">
              ...
              {twitterUrl && (
                <a href={twitterUrl} target="_blank" rel="noopener noreferrer" aria-label="X / Twitter" className="text-white/60 hover:text-white transition-colors">
